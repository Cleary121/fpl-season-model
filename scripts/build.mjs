...
H('BUDGET GEMS — enablers £4.5 and under, ranked by likelihood of playing');
// Likelihood: minutes share once the season is running; before that, crowd wisdom (ownership) + ep_next.
const gemScore = e => {
  const mShare = finishedGWs >= 3 ? Math.min(e.minutes / (finishedGWs * 90), 1) : Math.min(n(e.selected_by_percent) / 20, 1);
  return mShare * 3 + n(e.ep_next) + n(e.defensive_contribution_per_90) / 10;
};
const gems = bs.elements.filter(e => e.now_cost <= 45 && e.status === 'a' && e.element_type >= 2)
  .map(e => ({ e, g: gemScore(e) })).sort((a, b) => b.g - a.g).slice(0, 14);
head(['Player', 'Team', 'Pos', '£', 'ep_next', 'Mins', 'Owned%', 'DefCon/90', 'Next 3', 'Play-likelihood']);
gems.forEach(({ e, g }) => {
  const fx3 = (teamFx[e.team] || []).slice(0, 3).map(f => `${f.opp}(${f.ha})${f.fdr}`).join(' ');
  row([e.web_name, T[e.team], POS[e.element_type], money(e), e.ep_next, e.minutes, e.selected_by_percent, e.defensive_contribution_per_90, fx3, g.toFixed(1)]);
});
const bargainBin = gems.filter(x => x.e.now_cost <= 40).map(x => `${x.e.web_name} (${T[x.e.team]}, ${POS[x.e.element_type]}, £${money(x.e)})`);
L.push(bargainBin.length
  ? `\n**£4.0-or-less bargain bin (likely starters):** ${bargainBin.join(', ')}`
  : '\n**£4.0-or-less bargain bin:** none currently look like reliable starters.');
const cheapRisers = bs.elements.filter(e => e.now_cost <= 45 && e.status === 'a' && (e.transfers_in_event - e.transfers_out_event) > 20000)
  .sort((a, b) => (b.transfers_in_event - b.transfers_out_event) - (a.transfers_in_event - a.transfers_out_event)).slice(0, 5);
if (cheapRisers.length) L.push(`**Cheap players the crowd is buying now (price-rise watch):** ${cheapRisers.map(e => `${e.web_name} £${money(e)} (+${e.transfers_in_event - e.transfers_out_event})`).join(', ')}`);
...
// ---------- mobile HTML (GitHub Pages) ----------
const esc = s => String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
const mdInline = s => esc(s).replace(/\*\*(.+?)\*\*/g, '<b>$1</b>');
let body = '';
let inTable = false, inList = false;
const closeBlocks = () => {
  if (inTable) { body += '</table></div>'; inTable = false; }
  if (inList) { body += '</ul>'; inList = false; }
};
for (const ln of L.join('\n').split('\n')) {
  if (ln.startsWith('## ')) { closeBlocks(); body += `<h2>${esc(ln.slice(3))}</h2>`; }
  else if (ln.startsWith('# ')) { closeBlocks(); body += `<h1>${esc(ln.slice(2))}</h1>`; }
  else if (ln.startsWith('| ')) {
    const cells = ln.split('|').slice(1, -1).map(c => c.trim());
    if (cells.every(c => /^-+$/.test(c))) continue;
    if (!inTable) { closeBlocks(); body += '<div class="tw"><table><tr>' + cells.map(c => `<th>${mdInline(c)}</th>`).join('') + '</tr>'; inTable = true; }
    else body += '<tr>' + cells.map((c, ci) => `<td${ci === 0 ? ' class="pl"' : ''}>${mdInline(c)}</td>`).join('') + '</tr>';
  }
  else if (ln.startsWith('- ')) { if (inTable) closeBlocks(); if (!inList) { body += '<ul>'; inList = true; } body += `<li>${mdInline(ln.slice(2))}</li>`; }
  else if (ln.trim() === '') { closeBlocks(); }
  else { closeBlocks(); body += `<p>${mdInline(ln)}</p>`; }
}
closeBlocks();
const page = `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Premier FC — FPL Briefing</title>
<style>
:root{color-scheme:light dark;
--bg:#f9f9f7;--card:#fcfcfb;--ink:#0b0b0b;--ink2:#52514e;--muted:#898781;--line:#e1e0d9;--accent:#2a78d6}
@media (prefers-color-scheme:dark){:root{--bg:#0d0d0d;--card:#1a1a19;--ink:#fff;--ink2:#c3c2b7;--line:#2c2c2a;--accent:#3987e5}}
*{box-sizing:border-box;margin:0}
body{font-family:system-ui,-apple-system,"Segoe UI",sans-serif;background:var(--bg);color:var(--ink);padding:14px;font-size:15px;line-height:1.45;max-width:760px;margin:0 auto}
h1{font-size:19px;margin:4px 0 8px}
h2{font-size:15px;margin:22px 0 8px;padding:8px 10px;background:var(--card);border-left:3px solid var(--accent);border-radius:0 6px 6px 0;position:sticky;top:0}
p{margin:6px 0;color:var(--ink2);font-size:13.5px}
p b{color:var(--ink)}
ul{margin:6px 0 6px 18px;font-size:13.5px}
li{margin:4px 0}
.tw{overflow-x:auto;-webkit-overflow-scrolling:touch;margin:8px -14px;padding:0 14px}
table{border-collapse:collapse;font-size:12.5px;white-space:nowrap;min-width:100%}
th{font-size:10.5px;text-transform:uppercase;letter-spacing:.03em;color:var(--muted);text-align:left;padding:6px 8px;border-bottom:1px solid var(--line)}
td{padding:7px 8px;border-bottom:1px solid var(--line);font-variant-numeric:tabular-nums}
td.pl{font-weight:600;position:sticky;left:0;background:var(--bg)}
footer{margin:26px 0 10px;font-size:11.5px;color:var(--muted);text-align:center}
</style></head><body>
${body}
<footer>Rebuilt daily 06:00 UTC + Thu 15:30 UTC · <a href="https://github.com/Cleary121/fpl-season-model" style="color:var(--accent)">source</a></footer>
</body></html>`;
